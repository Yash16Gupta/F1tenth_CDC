...
public:
          ...
          tf_listener_(tf_buffer_)
    {
        ...
        max_speed_ = this->declare_parameter("max_speed", 11.0);
        min_speed_ = this->declare_parameter("min_speed", 7.0);
        max_lookahead_ = this->declare_parameter("max_lookahead", 1.6);
        min_lookahead_ = this->declare_parameter("min_lookahead", 1.3);
		...
		lidar_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
    		"/scan", 10, std::bind(&AdaptivePurePursuit::scan_callback, this, std::placeholders::_1));
        load_raceline_csv("/home/nvidia/Downloads/sahil_mc_raceline.csv");

        timer_ = this->create_wall_timer(
            std::chrono::milliseconds(1),
            std::bind(&AdaptivePurePursuit::publish_control_commands, this));
        los_direction_pub_ = this->create_publisher<visualization_msgs::msg::Marker>("/los_direction", 10);
    ...
    rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr los_direction_pub_;
    ...
    void publish_los_direction(const Eigen::Vector2d &goal_point)
{
    if (!current_position_.has_value())
    {
        RCLCPP_WARN(this->get_logger(), "Current position is not available. Cannot publish LOS direction.");
        return;
    }

    visualization_msgs::msg::Marker los_marker;
    los_marker.header.frame_id = "map";
    los_marker.header.stamp = this->get_clock()->now();
    los_marker.type = visualization_msgs::msg::Marker::ARROW;
    los_marker.action = visualization_msgs::msg::Marker::ADD;

    // Set start point of the arrow (current position)
    los_marker.pose.position.x = current_position_.value().x();
    los_marker.pose.position.y = current_position_.value().y();
    los_marker.pose.position.z = 0.0;

    // Calculate the direction vector
    Eigen::Vector2d direction = goal_point - current_position_.value();
    double yaw = std::atan2(direction.y(), direction.x());

    // Set orientation for the arrow based on yaw
    tf2::Quaternion q;
    q.setRPY(0, 0, yaw);
    los_marker.pose.orientation.x = q.x();
    los_marker.pose.orientation.y = q.y();
    los_marker.pose.orientation.z = q.z();
    los_marker.pose.orientation.w = q.w();

    // Set arrow scale
    los_marker.scale.x = direction.norm(); // Length of the arrow
    los_marker.scale.y = 0.05; // Thickness
    los_marker.scale.z = 0.05;

    // Set color
    los_marker.color.r = 1.0; // Red
    los_marker.color.g = 1.0; // Yellow for visibility
    los_marker.color.b = 0.0;
    los_marker.color.a = 1.0;

    los_direction_pub_->publish(los_marker);
}

// Function to check the line of sight
	bool is_line_of_sight_clear(const Eigen::Vector2d &goal_point)
	{

		if (lidar_ranges_.empty())
		{
		    RCLCPP_WARN(this->get_logger(), "No LiDAR data available for line-of-sight check.");
		    return false;
		}

		// Calculate angle to the goal point in the robot's local frame
		Eigen::Vector2d delta = goal_point - current_position_.value();
		double goal_angle = std::atan2(delta.y(), delta.x());
        std::cout<<"goal_angle: "<<goal_angle<<std::endl;
		// Convert the angle to LiDAR index
		double lidar_angle_min = -M_PI*(3.0/4.0); // Adjust based on your LiDAR's configuration
		double lidar_angle_increment = 1081; // Example: 1-degree resolution
		size_t goal_index = static_cast<size_t>(
		    (goal_angle - lidar_angle_min) / lidar_angle_increment);

		// Ensure index is within bounds
		if (goal_index >= lidar_ranges_.size())
		{
		    RCLCPP_WARN(this->get_logger(), "Goal point out of LiDAR scan range.");
		    return false;
		}

		// Check if there are obstacles within the range to the goal point
		double goal_distance = delta.norm();
		if (lidar_ranges_[goal_index] < goal_distance)
		{
		    RCLCPP_INFO(this->get_logger(), "Obstacle detected between robot and goal point.");
		    return false;
		}
	...
	current_position_ = Eigen::Vector2d(transform.transform.translation.x, transform.transform.translation.y);
        ...
        publish_los_direction(goal_point.value());
		bool r=is_line_of_sight_clear(goal_point.value());
