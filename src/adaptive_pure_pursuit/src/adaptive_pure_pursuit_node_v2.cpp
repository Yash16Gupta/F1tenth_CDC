...
public:
          ...
          tf_listener_(tf_buffer_)
    {
        ...
        a_ = this->declare_parameter("a_", 1);
        r_ = this->declare_parameter("r_", 0.8);
    ...
    double lookahead_distance_, beta_, previous_deviation_, total_area_, control_velocity_, heading_angle_, r_, a_;
    ...
    double convex_combination(double max_velocity_pp, double min_deviation_pp, double current_speed, double area)
    {
        ...
        velocities_.push_back(control_velocity);
        ...
        for (auto velocity_it = velocities_.rbegin(); velocity_it != velocities_.rend(); ++velocity_it, ++weight_it) {
        moving_avg += (*velocity_it) * (*weight_it);
        }
