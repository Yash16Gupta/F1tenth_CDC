...
options = {
  ...
  pose_publish_period_sec = 0.01,  -- attitude release period (seconds)
  trajectory_publish_period_sec = 0.01,  -- trajectory release period (seconds)
  rangefinder_sampling_ratio = 1.,  -- rangefinder sampling ratio
  odometry_sampling_ratio = 0.1,  -- odometer sampling rate
  fixed_frame_pose_sampling_ratio = 1.,  -- fixed frame attitude sampling ratio  
  imu_sampling_ratio = 1.0,  -- IMU sampling ratio
  landmarks_sampling_ratio = 1.,  -- landmarks sampling ratio
}
 
MAP_BUILDER.use_trajectory_builder_2d = true  -- whether use 2D SLAM
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 10  -- Number of range data for submaps in the 2D track builder  
...
TRAJECTORY_BUILDER_2D.max_range = 5  -- the maximum scanning range of the lidar
...
TRAJECTORY_BUILDER_2D.motion_filter.max_angle_radians = math.rad(1.0)  -- Modify 1.0 to 0.1, increased sensitivity to movement
...
POSE_GRAPH.constraint_builder.min_score = 0.60  -- Modify 0.55 to 0.65, the minium score of Fast csm, can be optimized above this score 
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.60  -- Modify 0.6 as 0.7, Minimum global positioning score below which global positioning is considered currently inaccurate

--TRAJECTORY_BUILDER_2D.submaps.grid_options_2d.resolution = 0.08  -- Default is 0.05; larger values reduce detail and computational cost
