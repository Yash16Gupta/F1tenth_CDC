...
class MuxNode(Node):
    def __init__(self):
        ...
        self.last_b_to_a_switch_time = None

        # Constants
        self.safe_distance = 1.0  # Threshold to detect obstacles in m
        self.detection_angle = 60  # Field of view for obstacle detection in degree
        self.instance_threshold = 5  # Number of points
        self.declare_parameter('node_switch_delay', 1.0)  # Delay in seconds for switching from Node B to Node A
    ...
    def lidar_callback(self, scan_msg):
        ...
        if obstacle_detected:
            ...
            if self.use_node_b:
                # Add delay when switching from Node B to Node A
                if self.last_b_to_a_switch_time is None:
                    self.last_b_to_a_switch_time = time.time()

                switch_delay = self.get_parameter('node_switch_delay').get_parameter_value().double_value
                current_time = time.time()
                if current_time - self.last_b_to_a_switch_time >= switch_delay:
                    self.get_logger().info("Switching to Node A after delay.")
                    self.use_node_b = False
                    self.last_b_to_a_switch_time = None  # Reset the timer
            else:
